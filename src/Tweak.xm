        ...
        if ([contactMgr respondsToSelector:isInContactListSelector]) {
            ...
            if (!isFollowing) {
                ...
                UIAlertAction *followAction = [UIAlertAction actionWithTitle:@"去关注"
                    ...
                    // 跳转到微信公众号页面
                    NSURL *githubUrl = [NSURL URLWithString:@"https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=Mzg5OTkwNTc4Mg==&subscene=0#wechat_redirectt"];
...
- (void)viewDidLoad {
    ...
    // 修改导航栏标题
    [(UIViewController *)self navigationItem].title = @"Misaka";
}
...
- (void)viewDidLoad {
    ...
    // 修改导航栏标题
    [(UIViewController *)self navigationItem].title = @" 娱乐";
}
...
- (void)openMovie {
    ...
    // 跳转到微信公众号页面
    NSURL *url = [NSURL URLWithString:@"https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=Mzg5OTkwNTc4Mg==&subscene=0#wechat_redirectt"];
    MMWebViewController *webViewController = [[objc_getClass("MMWebViewController") alloc] initWithURL:url presentModal:NO extraInfo:nil];
    [((UIViewController *)self).navigationController pushViewController:webViewController animated:YES];
}
    ...
    if (NSClassFromString(@"WCPluginsMgr")) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if (NSClassFromString(@"WCPluginsMgr") && !didRegisterXUUZHelper) {
                ...
                // 在插件管理注册带设置页面的插件
                [pluginsMgr registerControllerWithTitle:@"Mieing" version:@"5.2.0" controller:@"WCPLSettingViewController"];
                didRegisterXUUZHelper = YES;
            }
        });
    }
