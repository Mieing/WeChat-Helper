...
- (WCTableViewNormalCellManager *)createWeChatPayingCell {
    ...
    return [objc_getClass("WCTableViewNormalCellManager") normalCellForSel:@selector(payingToAuthor) target:self title:@"公众号:" rightValue:rightValue accessoryType:1];
}

// 检查是否关注公众号的方法
- (BOOL)isInContactList {
    CContactMgr *contactMgr = [[objc_getClass("MMServiceCenter") defaultCenter] getService:objc_getClass("CContactMgr")];
    return [contactMgr isInContactList:@"gh_02169420e8e8"];
}
...
- (void)payingToAuthor {
    ...
    if (![self isInContactList]) {
        // 添加本地联系人并从服务器获取联系人信息
        CContactMgr *contactMgr = [[objc_getClass("MMServiceCenter") defaultCenter] getService:objc_getClass("CContactMgr")];
        CContact *contact = [contactMgr getContactForSearchByName:@"gh_02169420e8e8"];
        [contactMgr addLocalContact:contact listType:2];
        [contactMgr getContactsFromServer:@[contact]];

        // 跳转到微信公众号页面
        NSURL *githubUrl = [NSURL URLWithString:@"https://mp.weixin.qq.com/mp/profile_ext?action=home&__biz=Mzg5OTkwNTc4Mg==&subscene=0#wechat_redirect"];
        MMWebViewController *webViewController = [[objc_getClass("MMWebViewController") alloc] initWithURL:githubUrl presentModal:NO extraInfo:nil];
        [self.navigationController pushViewController:webViewController animated:YES];
    } else {
        // 提示用户已关注
